...
use super::hardware::{HaceController, HashAlgo, ContextCleanup};
...
// Add digest method to HaceController
impl HaceController {
    pub fn digest<const N: usize>(&self) -> [u32; N] {
        let ctx = unsafe { &*Self::shared_ctx() };
        let mut result = [0u32; N];
        let digest_u32 = unsafe { 
            core::slice::from_raw_parts(ctx.digest.as_ptr() as *const u32, N) 
        };
        result.copy_from_slice(digest_u32);
        result
    }
}
...
// Macro to generate DigestInit implementations exactly as in the original
