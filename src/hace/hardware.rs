...
impl HaceController {
    ...
    /// Initialize the context for the selected algorithm
    pub fn init_context(&mut self) -> Result<(), HardwareError> {
        let algo = self.algo;
        let ctx = self.ctx_mut();
        
        ctx.method = algo.hash_cmd();
        ctx.iv_size = algo.iv_size() as u8;
        ctx.bufcnt = 0;
        ctx.digcnt = [0; 2];
        ctx.buffer.fill(0);
        ctx.digest.fill(0);
    ...
    pub fn hash_key(&mut self, key: &impl AsRef<[u8]>) {
        ...
        let ctx = self.ctx_mut();
        ctx.digcnt[0] = key_len as u64;
        ctx.bufcnt = u32::try_from(key_len).expect("key_len too large to fit in u32");
        ctx.buffer[..key_len].copy_from_slice(key_bytes);
        ...
        let ctx = self.ctx_mut();
        let slice =
            unsafe { core::slice::from_raw_parts(ctx.digest.as_ptr(), digest_len) };

        ctx.key[..digest_len].copy_from_slice(slice);
        ctx.ipad[..digest_len].copy_from_slice(slice);
        ctx.opad[..digest_len].copy_from_slice(slice);
        ctx.key_len =
    ...
    /// Update hash with new data
    pub fn update(&mut self, data: &[u8]) -> Result<(), HardwareError> {
        ...
        // Update digest count
        {
            ...
            ctx.digcnt[0] = old_count.wrapping_add(data_len as u64);
            if ctx.digcnt[0] < old_count {
                ctx.digcnt[1] = ctx.digcnt[1].wrapping_add(1);
            }
        }
        ...
        let should_process_buffer = {
            let ctx = self.ctx_mut();
            if ctx.bufcnt > 0 {
                let remaining = block_size - ctx.bufcnt as usize;
                let copy_len = core::cmp::min(remaining, data_len);
                
                ctx.buffer[ctx.bufcnt as usize..ctx.bufcnt as usize + copy_len]
                    .copy_from_slice(&data[..copy_len]);
                ctx.bufcnt += copy_len as u32;
                offset += copy_len;

                if ctx.bufcnt as usize == block_size {
                    ctx.method &= !HACE_SG_EN; // Use buffer mode
                    true // Signal that we need to process the buffer
                } else {
                    false
                }
            } else {
        ...
        while offset + block_size <= data_len {
            {
                let ctx = self.ctx_mut();
                ctx.buffer[..block_size].copy_from_slice(&data[offset..offset + block_size]);
                ctx.method &= !HACE_SG_EN; // Use buffer mode
            }
            self.start_hash_operation(block_size as u32);
            offset += block_size;
        }
        ...
        if offset < data_len {
            ...
            ctx.bufcnt = remaining as u32;
        }

        Ok(())
    }
    pub fn finalize(&mut self) -> Result<&[u8], HardwareError> {
        self.fill_padding(0);
        let bufcnt = self.ctx_mut().bufcnt;
        self.start_hash_operation(bufcnt);
