...
pub fn test_i2c_master(uart: &mut UartController<'_>) {
    ...
    let i2c_config = I2cConfigBuilder::new()
        ...
        .build();
    let mut i2c3: I2cController<
        Ast1060I2c<ast1060_pac::I2c3, DummyI2CTarget, UartLogger>,
        NoOpLogger,
    > = I2cController {
    ...
    pinctrl::Pinctrl::apply_pinctrl_group(pinctrl::PINCTRL_I2C3);
    i2c3.hardware.init(&mut i2c3.config);

    let addr = 0x2e; //device ADT7490
    let mut buf = [0x4e];
    if true {
        match i2c3.hardware.write(addr, &buf) {
            Ok(val) => {
        ...
        match i2c3.hardware.read(addr, &mut buf) {
            Ok(val) => {
        ...
        if false {
            ...
            let buf2 = [0x82, 0x3];
            match i2c3.hardware.write(addr, &buf2) {
                Ok(val) => {
                    writeln!(uart, "i2c write ok: {val:?}\r").unwrap();
                }
                Err(e) => {
